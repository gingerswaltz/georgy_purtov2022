class Pyramid {
  ...
  calculateVertices() {
    ...
    const topVertex = new Point(this.y, this.x, 300, 0.5, 0.5); // Центральные текстурные координаты

    vertices.push(topVertex);
    for (let i = 0; i < baseVertexCount; i++) {
      const u = (this.baseVertices[i].x - this.x + 200) / 400; // Пример расчета u относительно положения вершины на оси X
      const v = (this.baseVertices[i].y - this.y + 200) / 400; // Пример расчета v относительно положения вершины на оси Y
      vertices.push(
        new Point(
  ...
  rotate(deg, t0) {
    for (let vertex of this.vertices) {
      // Проход по всем вершинам модели.
  ...
  projection(type, tv, d) {
    ...
    for (let face of this.faces) {
      ...
      for (let vertex of face) {
        // Проход по вершинам текущей грани.
...
class Point {
  constructor(x, y, z, u, v) {
    ...
    this.y = y;
    this.z = z;
    this.u = u; // Текстурная координата u
    this.v = v; // Текстурная координата v
  }
...
// Отрисовка грани с учетом текстуры
function drawFaceWithTexture(context, face, texture) {
  context.beginPath();
  for (let j = 0; j < face.length; j++) {
    ...
    const u = vertex.u; // Получение текстурной координаты u.
    const v = vertex.v; // Получение текстурной координаты v.
  ...
  // Создание паттерна с текстурой после ее загрузки
  const pattern = context.createPattern(texture, 'repeat');
  context.fillStyle = pattern; // Использование текстуры для заливки
  context.fill(); // Заливка контура текстурой
  context.stroke(); // Отрисовка контура.
}
// Функция для перерисовки изображения с учетом текстуры
function repaint() {
  if (pyramid && t0) { // Проверка на существование pyramid и t0
    pyramid.rotate(deg, t0); // Поворот пирамиды.
    pyramid.calculateNormals(); // Расчет нормалей к граням.

    const texture = new Image(); // Создание объекта изображения для текстуры
    texture.src = 'texture.png'; // Путь к текстуре
    texture.onload = function() { // Обработчик события загрузки текстуры
      drawFigureWithTexture(canvas1, pyramid.projection(tv), texture); // Отрисовка пирамиды с учетом текстуры
    };
  }
}
// Функция для отрисовки пирамиды на canvas с учетом текстуры
function drawFigureWithTexture(canvas, proj, texture) {
  let context = canvas.getContext('2d'); // Получение контекста рисования.
  context.clearRect(0, 0, canvas.width, canvas.height); // Очистка canvas.

  // Отрисовка каждой грани с учетом текстуры
  for (let i = 0; i < proj.length; i++) {
    ...
    // Отрисовка грани с учетом текстуры
    drawFaceWithTexture(context, face, texture);
  }
}
